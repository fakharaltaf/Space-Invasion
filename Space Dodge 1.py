...
class ship():
    ...
    def shoot(self):
        if self.cooldowntimer == 0:
            laser = Laser(self.x , self.y , self.laser, self.speed)
...
def collision(obj1, obj2):
    ...
    return obj1.mask.overlap(obj2, (xoffset,yoffset)) != None
...
def main():
    ...
    enemies_on_screen = []
    enemy_wave = 10
    player = Player(500,333,5)
    gameover = False
    lost_timer = 0
    
    # Creating Enemies Objects
    for i in range(enemy_wave):
        enemies_on_screen.append(Enemy(random.randrange(0,900),random.randrange(-5000*((player_level+1)/5),0),5))
